...
function renderCell(cell: string | number, column = '') {
  if (typeof cell === 'number') {
    ...
    if (/contract count|deal count|contracts|records|unmatched/i.test(column)) return formatCount(cell);
...
type MetricMode = 'dollars' | 'acres' | 'deals';
const metricLabel = (metric: MetricMode) => metric === 'dollars' ? 'Dollars' : metric === 'acres' ? 'Acres' : 'Deal Count';
...
export function IncomeStatement({ filters }: FinancePageProps)
{
  ...
  const grossProfit = {
    current: totalRevenue.current + costOfRevenue.current,
    prior: totalRevenue.prior + costOfRevenue.prior,
    ytd: totalRevenue.ytd + costOfRevenue.ytd,
    priorYtd: totalRevenue.priorYtd + costOfRevenue.priorYtd,
  };
  ...
  const departmentWeight = expenseByDepartment.reduce((sum, item) => sum + item.value, 0);
  const departmentExpenseRows = expenseByDepartment.map((item) => ({
    id: `department-${item.department}`,
    label: item.department,
    current: Math.round(totalOperatingExpenses.current * item.value / departmentWeight),
    prior: Math.round(totalOperatingExpenses.prior * item.value / departmentWeight),
  ...
  const profitAndLossRows: ProfitAndLossRow[] = [
    ...
    { id: 'costOfGoodsSold', label: 'Cost of Goods Sold (COGS)', ...costOfRevenue },
    { id: 'grossProfit', label: 'Gross Profit', ...grossProfit },
    { id: 'operatingExpenses', label: 'Operating Expenses', ...totalOperatingExpenses, children: departmentExpenseRows },
    ...
    >
      ...
      <div className="bg-white border border-[#CFD5D0] p-4">
        <div className="grid grid-cols-3 gap-3">
          ...
          <PowerBISlicer title="Metric" value={recognitionMetric} onChange={(value) => setRecognitionMetric(value as MetricMode)} options={[{ value: 'dollars', label: 'Dollars' }, { value: 'acres', label: 'Acres' }, { value: 'deals', label: 'Deal Count' }]} />
      ...
      <div className="grid grid-cols-2 items-start gap-3">
        {stageView === 'chart' ? (
          <ChartCard title="Monthly Stage Reporting" subtitle={`${metricLabel(recognitionMetric)} by recognition period and stage`} height={210}>
            <BarChart data={visiblePeriodData}>
              ...
              <YAxis tick={chartText} tickFormatter={recognitionMetric === 'dollars' ? moneyAxis : recognitionMetric === 'deals' ? (value) => String(value) : (value) => `${(Number(value) / 1000).toFixed(0)}K`} />
              <Tooltip contentStyle={{ fontFamily: 'Source Sans 3, sans-serif' }} formatter={(value: number) => recognitionMetric === 'dollars' ? moneyTip(value) : recognitionMetric === 'deals' ? `${formatCount(Number(value))} Deals` : `${formatAcres(Number(value))} Acres`} />
              <Legend wrapperStyle={chartText} />
        ...
        <FinanceTable title="Earned Revenue by Stage" subtitle="Stage-level dollars, acres, and deal count" columns={['Earned Revenue Stage', 'Dollars', 'Acres', 'Deal Count']} rows={visibleStageRows} />
      </div>
...
export function BalanceSheet({ filters }: FinancePageProps)
{
  ...
  const retainedEarnings = totalEquity - contributedCapital - currentPeriodEarnings - distributions;
  const priorRetainedEarnings = priorTotalEquity - priorContributedCapital - priorCurrentPeriodEarnings - priorDistributions;
  ...
  const rows: StatementRow[] = [
    ...
    },
    {
      ...
      children: [
        ...
        { id: 'distributions', label: 'Distributions', current: distributions, prior: priorDistributions },
      ],
    },
  ];
  const toggleExpandedRow = (rowId: string) => {
      ...
      <div className="grid grid-cols-2 gap-3">
        <FinanceTable
          ...
          rows={[
            ...
            ['Soil Vendor Payables', vendorPayables],
          ]}
          onDrillDown={(rowLabel, column, value, sourceTitle) => setVendorPayablesDrillDown({ rowLabel, column, value, sourceTitle })}
          drillDownCells={[{ rowLabel: 'Soil Vendor Payables', column: 'Amount' }]}
        />
